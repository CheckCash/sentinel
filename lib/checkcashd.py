...
class CheckCashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some checkcashd methods
    ...
    @classmethod
    def from_checkcash_conf(self, checkcash_dot_conf):
        from checkcash_config import CheckCashConfig
        config_text = CheckCashConfig.slurp_config_file(checkcash_dot_conf)
        creds = CheckCashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = checkcashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = checkcashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        DO NOT CALL DIRECTLY if you don't checkcash a "Oh Noes." exception.
        """
        current_block_height = self.rpc_command('getblockcount')
        ...
        future_seconds = checkcashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_checkcashd_proto_version_with_sentinel_ping)
