...
def has_checkcash_conf():
    import config
    import io

    valid_checkcash_conf = False

    # ensure checkcash_conf exists & readable
    #
    # if not, print a message stating that CheckCash Core must be installed and
    # configured, including JSONRPC access in checkcash.conf
    try:
        f = io.open(config.checkcash_conf)
        valid_checkcash_conf = True
    except IOError as e:
        print(e)

    return valid_checkcash_conf
...
def main():
    ...
    if not has_checkcash_conf():
        print("CheckCash must be installed and configured, including JSONRPC access in checkcash.conf")
        sys.exit(1)
