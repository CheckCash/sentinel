...
class GovernanceClass(object):
    ...
    def vote(self, checkcashd, signal, outcome):
        return self.go.vote(checkcashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, checkcashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(checkcashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(checkcashd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to checkcashd "gobject list" output
    ...
    def checkcashd_serialise(self):
        import checkcashlib
        return checkcashlib.SHIM_serialise_for_checkcashd(self.serialise())
