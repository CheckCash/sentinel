...
def is_valid_checkcash_address(address, network='mainnet'):
    ...
    checkcash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid CheckCash address
        return False

    if (address_version != checkcash_version):
        return False
...
# shims 'til we can fix the checkcashd side
def SHIM_serialise_for_checkcashd(sentinel_hex):
    ...
    # superblock => "trigger" in checkcashd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # checkcashd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    checkcashd_hex = serialise(obj)
    return checkcashd_hex


# shims 'til we can fix the checkcashd side
def SHIM_deserialise_from_checkcashd(checkcashd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(checkcashd_hex)

    # shim from checkcashd
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('checkcash.conf')
