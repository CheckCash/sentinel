...
sentinel_cfg = CheckCashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_checkcashd_proto_version_with_sentinel_ping = 70207


def get_checkcash_conf():
    home = os.environ.get('HOME')

    checkcash_conf = os.path.join(home, ".checkcash/checkcash.conf")
    if sys.platform == 'darwin':
        checkcash_conf = os.path.join(home, "Library/Application Support/CheckCash/checkcash.conf")

    checkcash_conf = sentinel_cfg.get('checkcash_conf', checkcash_conf)

    return checkcash_conf
...
checkcash_conf = get_checkcash_conf()
network = get_network()
db = get_db_conn()
