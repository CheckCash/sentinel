...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, checkcashd):
        golist = checkcashd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_checkcashd(checkcashd, item)
    ...
    @classmethod
    def import_gobject_from_checkcashd(self, checkcashd, rec):
        import decimal
        import checkcashlib
        import inflection
        ...
        # shim/checkcashd conversion
        object_hex = checkcashlib.SHIM_deserialise_from_checkcashd(object_hex)
        objects = checkcashlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from checkcashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from checkcashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from checkcashd - CheckCashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(checkcashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from checkcashd! %s" % e)
            govobj.vote_delete(checkcashd)
            return (govobj, None)
    ...
    def vote_delete(self, checkcashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(checkcashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, checkcashd, signal, outcome):
        import checkcashlib
        ...
        output = checkcashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = checkcashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(checkcashd, signal)

    def sync_network_vote(self, checkcashd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = checkcashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 checkcash addr, non-multisig
            if not checkcashlib.is_valid_checkcash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid CheckCash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (checkcashlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CheckCashDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import checkcashlib
        obj_data = checkcashlib.SHIM_serialise_for_checkcashd(self.serialise())
    ...
    def prepare(self, checkcashd):
        try:
            object_hash = checkcashd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not checkcashlib.is_valid_checkcash_address(addr, config.network):
    ...
    def hash(self):
        import checkcashlib
        return checkcashlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, checkcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - checkcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, checkcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - checkcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, checkcashd):
        now = int(time.time())
        return (self.created_at < (now - checkcashd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, checkcashd):
        if self.is_expired(checkcashd):
            return False

        return True

    def is_deletable(self, checkcashd):
        if self.is_expired(checkcashd):
            return True
